...
│   ├── templates/relatorios/   base.html, lista_relatorios.html, cadastrar_relatorio.html
...
A `SECRET_KEY` e as demais credenciais não são versionadas: ficam no arquivo `.env`, bloqueado pelo `.gitignore`. O arquivo `.env.example` documenta quais variáveis são necessárias, sem expor seus valores.
